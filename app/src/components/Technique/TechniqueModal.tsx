...
interface TechniqueModalProps {
  onSuccess?: (data: { name: string; description: string; category_id: number; linked_technique: number }) => void;
  technique?: Technique,
...
export function TechniqueModal({
  ...
  const [categoryId, setCategoryId] = useState(technique?.category.name ?? "");
  const [linkedTechnique, setLinkedTechnique] = useState(technique?.linked_technique.name ?? "");
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      if (technique?.id) {
            await editTechnique({id: technique.id, name, description, category_id: Number(categoryId), linked_technique: Number(linkedTechnique)})
      } else {
          await createTechnique({ name, description, category_id: Number(categoryId), linked_technique: Number(linkedTechnique)});
      }

      onSuccess?.({ name, description, category_id: Number(categoryId), linked_technique: Number(linkedTechnique) });
      setOpen(false);
      setName("");
      setDescription("");
  ...
  return (
    <Dialog open={open} onOpenChange={setOpen}>
      <DialogTrigger asChild>
        {trigger ?? (
          <Button variant="outline">Adicionar nova técnica de técnica</Button>
        )}
      </DialogTrigger>
              ...
              <SelectContent>
                <SelectGroup>
                  {categories.map((category) => (
                    <SelectItem value={category.id.toString()} required={true}>{category.name}</SelectItem>
                  ))}
                </SelectGroup>
              </SelectContent>
            </Select>
